#!/usr/bin/env python3
import sys, os, json, re, hashlib, logging
...
class NoteClassifier:
    def __init__(self):
        ...
        self.link_finder = LinkFinder(self.config['tag_registry']['path'], self.config.get('linking', {}))
    ...
    def _body_cache_key(self, fp):
        return hashlib.sha256(str(fp).encode()).hexdigest()

    def _read_body_cache(self, fp):
        cf = HASH_DIR / (self._body_cache_key(fp) + '.body')
        return cf.read_text(encoding='utf-8') if cf.exists() else ''

    def _write_body_cache(self, fp, body):
        (HASH_DIR / (self._body_cache_key(fp) + '.body')).write_text(body, encoding='utf-8')

    def _get_existing_ai_tags(self, content):
        return self.yaml_updater.parse_frontmatter(content).get('ai_tags', [])

    def _build_system_prompt(self):
        min_obs = self.config.get('learning', {}).get('min_observations', 1)
        top = self.tag_manager.get_top_tags(self.config['learning'].get('max_tags_in_prompt', 25), min_obs)
    ...
    def classify_note(self, file_path):
        try:
            ...
            related = self.link_finder.find_related(file_path, validated['tags'], validated['category'], validated['key_topics'])
            related_links = [self.link_finder.build_wikilink(r['path']) for r in related]
            fields = {
                'tags': validated['tags'],
            ...
            if related:
                updated_content = self.link_finder.inject_body_links(updated_content, related)
            ...
            return {
                'status': 'success',
                'action': 'reclassified' if already else 'classified',
                'tags': validated['tags'],
                'category': validated['category'],
