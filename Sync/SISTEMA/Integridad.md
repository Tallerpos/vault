...
Este panel centraliza la reparación de tu sistema de lectura sin necesidad de scripts externos, protegiendo tu gráfico de conocimiento de nodos fantasmas.
...
const brokenIdeas = ideas.filter(i => {
  const f = i.fuente;
  if (!f) return false;
  // Buscamos si NO tiene corchetes
